...
rm snap-* || true &&
wget http://snapshots-wtf.sensecapmx.cloud/snap-`curl 'http://snapshots-wtf.sensecapmx.cloud/latest-snap.json' | sed -e 's/[{}]/''/g' |  awk -v k="text" '{n=split($0,a,",");  print a[2]}' | awk -v k="text" '{n=split($0,b,":");  print b[2]}' | xargs` &&
...
docker exec miner miner snapshot load /var/data/snap/snap-`curl 'http://snapshots-wtf.sensecapmx.cloud/latest-snap.json' |
    sed -e 's/[{}]/''/g' | 
